...
namespace Kunstmaan\TranslatorBundle\Twig;
...
/**
 * Class KunstmaanTwigLexer
 */
class KunstmaanTwigLexer extends Twig_Lexer
{
    private $tokens;
    private $code;
    private $cursor;
    ...
    /**
     * KunstmaanTwigLexer constructor.
     *
    ...
    public function tokenize(Twig_Source $source)
    {
        ...
        $pattern = '/(?<!["|\']){{\s*([A-Za-z0-9._\-\'|"]*)(\s*\|\s*trans)(.*)\s*}}/m';
        $this->code = preg_replace_callback($pattern, function($matches) {
            return '<inline-trans class="inline-trans" data-keyword="' . trim($matches[1], '"\'') . '">' . $matches[0] . '</inline-trans>';
        },  $this->code);
